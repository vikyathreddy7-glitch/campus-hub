...
const AppContent: React.FC = () => {
  ...
  const handleDeleteItem = async (id: string) => {
    ...
    // Perform a soft delete (archive in original tables)
  ...
  const handlePermanentDelete = async (id: string) => {
    ...
    try {
      // Pass the title and user ID to ensure it is stored in the deleted archive log
      await supabaseService.deleteItem(id, item.type, item.title, currentUser.id);
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col pb-20">
      ...
      <main className="flex-grow">
        <Routes>
          <Route path="/" element={<Home items={items} onOpenChat={(id) => { const item = items.find(i=>i.id===id); if(item) setActiveOtherUserId(item.posterId); }} onViewDetail={setViewDetailItemId} currentUser={currentUser} />} />
          ...
          <Route path="/my-listings" element={<MyListings items={items.filter(i => i.posterId === currentUser.id)} onDelete={handleDeleteItem} onRestore={handleRestoreItem} onPermanentDelete={handlePermanentDelete} currentUser={currentUser} onViewDetail={setViewDetailItemId} />} />
        </Routes>
      </main>
