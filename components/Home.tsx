        ...
        <div className="relative overflow-hidden rounded-[2.5rem] shadow-xl h-[280px] md:h-[340px] group border border-gray-100">
          {CAROUSEL_SLIDES.map((slide, index) => (
            ...
            >
              ...
              {/* Text Content Layer */}
              <div className="relative z-20 px-8 w-full max-w-2xl">
                <div className="flex items-center gap-3 mb-2">
                   {index !== 0 && (
                     <span className="text-2xl drop-shadow-lg filter brightness-125">{slide.icon}</span>
                   )}
                   <h2 className={`text-3xl md:text-5xl font-black text-white tracking-tighter drop-shadow-2xl ${index === 0 ? 'ml-0' : ''}`}>
                    {slide.title}
                   </h2>
                </div>
                
                <p className="text-[13px] md:text-lg font-extrabold text-white/95 leading-relaxed mb-6 max-w-[320px] md:max-w-[480px] drop-shadow-md">
                  {slide.subtitle}
                </p>

                {slide.footer && (
                  <div className="inline-flex items-center gap-2 bg-white/95 backdrop-blur-md px-4 py-2.5 rounded-2xl text-[11px] font-black text-[#166534] shadow-xl border border-white">
                    <div className="w-4 h-4 bg-[#22C55E] rounded-full flex items-center justify-center text-white text-[9px]">
                      ✓
                    </div>
                    {slide.footer}
                  </div>
                    ...
                    <div className="mt-3 flex gap-1.5">
                      {isOwner ? (
                        <button 
                          ...
                          className="w-full bg-blue-50 text-blue-600 text-[9px] font-black py-2.5 rounded-xl border border-blue-100 hover:bg-blue-100 transition-colors uppercase tracking-widest"
                        >
                          View Detail
                        </button>
                      ) : (
