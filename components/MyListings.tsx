...
const MyListings: React.FC<MyListingsProps> = ({ items, onDelete }) => {
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm('Move this listing to history? It will be removed from the marketplace.')) return;
    setIsDeleting(id);
    try {
      await onDelete(id);
    } catch (err) {
    ...
    <div className="min-h-screen bg-[#FAF9FF] flex flex-col pb-24">
      <header className="px-6 pt-10 pb-6 sticky top-0 bg-[#FAF9FF] z-10">
        ...
        <div className="flex bg-white/50 backdrop-blur-md p-1.5 rounded-2xl border border-gray-100 shadow-sm">
          <button onClick={() => setActiveTab('active')} className={`flex-1 py-3 text-xs font-black uppercase tracking-widest rounded-xl transition-all ${activeTab === 'active' ? 'bg-[#2D4A8A] text-white shadow-lg' : 'text-gray-400'}`}>
            Active ({activeItems.length})
          </button>
          <button onClick={() => setActiveTab('history')} className={`flex-1 py-3 text-xs font-black uppercase tracking-widest rounded-xl transition-all ${activeTab === 'history' ? 'bg-[#2D4A8A] text-white shadow-lg' : 'text-gray-400'}`}>
            ...
            activeItems.map(item => (
              <div key={item.id} className="bg-white rounded-[2rem] border border-gray-100 p-4 shadow-sm flex gap-4 animate-in slide-in-from-bottom-2 duration-300">
                ...
                <div className="flex-grow min-w-0 flex flex-col justify-between py-1">
                  ...
                  >
                    {isDeleting === item.id ? <div className="w-3 h-3 border-2 border-red-200 border-t-red-600 rounded-full animate-spin" /> : 'Remove Listing'}
                  </button>
                </div>
              </div>
            ))
              ...
              <div key={item.id} className="bg-white rounded-[2rem] border border-gray-100 p-4 shadow-sm flex gap-4 opacity-75">
                <div className="w-24 h-24 rounded-2xl overflow-hidden flex-shrink-0 relative">
                   ...
                   <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
                      <span className={`text-[10px] font-black text-white border-2 border-white px-2 py-1 rounded-lg uppercase rotate-[-12deg] ${item.status === ItemStatus.DELETED ? 'bg-red-600/50' : ''}`}>
                        {item.status === ItemStatus.DELETED ? 'DELETED' : item.status}
                      </span>
                   </div>
                </div>
                <div className="flex-grow min-w-0 flex flex-col justify-center py-1">
                   ...
                   <p className="text-[10px] text-gray-400 font-bold uppercase mt-1">
                     {item.status === ItemStatus.DELETED ? 'Listing removed by owner' : item.status === ItemStatus.SOLD ? `Sold for ₹${item.price}` : 'Recovered'}
                   </p>
                </div>
              </div>
            ))
          ) : <EmptyState icon="📂" message="No history yet" submessage="Successfully sold, recovered, or removed items will be listed here." />
        )}
      </div>
    </div>
  );
